...
def html_to_markdown(html_content: str) -> str:
    ...
    html_content = re.sub(r'<a[^>]*href="([^"]*)"[^>]*>(.*?)</a>', clean_link, html_content, flags=re.DOTALL | re.IGNORECASE)
    ...
    html_content = re.sub(r'<img[^>]*src="([^"]*)"[^>]*alt="([^"]*)"[^>]*>', convert_image, html_content, flags=re.IGNORECASE)
    html_content = re.sub(r'<img[^>]*src="([^"]*)"[^>]*>', '![Image](\\1)', html_content, flags=re.IGNORECASE)
    
    # Convert strong/bold
    html_content = re.sub(r'<(?:strong|b)[^>]*>(.*?)</(?:strong|b)>', '**\\1**', html_content, flags=re.DOTALL | re.IGNORECASE)
